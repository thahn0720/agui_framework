...
import thahn.java.agui.animation.AccelerateInterpolator;
import thahn.java.agui.animation.AlphaAnimation;
import thahn.java.agui.animation.Animation;
import thahn.java.agui.animation.Animation.AnimationListener;
import thahn.java.agui.animation.AnimationSet;
import thahn.java.agui.animation.AnimationUtils;
import thahn.java.agui.animation.DecelerateInterpolator;
import thahn.java.agui.animation.Interpolator;
import thahn.java.agui.animation.ScaleAnimation;
...
import agui.support.v4.util.DebugUtils;
import agui.support.v4.util.LogWriter;
...
public final class FragmentManagerImpl extends FragmentManager {
    ...
    static final boolean HONEYCOMB = true;//android.os.Build.VERSION.SDK_INT >= 11;
    ...
    Runnable mExecCommit = new Runnable() {
        ...
        public void run() {
            execPendingActions();
            // this is agui's feature
            // removed this code in add and then added here. later arrange
            mActivity.arrangeView();
            // this is agui's feature
...
//                                f.mView = NoSaveStateFrameLayout.wrap(f.mView);
                                if (container != null) {
                                    ...
                                    if(f.mView.getParent() == null) {
                                    	container.addView(f.mView);
                                    } else if(f.mView.getParent() != container) {
                                    	throwException(new WrongFormatException("fragment already has a parent."));
                                    }
                                }
                                if (f.mHidden) f.mView.setVisibility(View.GONE);
                                f.onViewCreated(f.mView, f.mSavedFragmentState); 
