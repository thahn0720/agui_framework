...
import agui.support.v4.view.VelocityTrackerCompat;
import agui.support.v4.view.ViewCompat;
...
//            final int childWidth = mCapturedView.getWidth();
//            final int childHeight = mCapturedView.getHeight();
//            dx = dx - childWidth;
            
            if (dx != 0) {
//            	Log.i("dx : " + (dx - childWidth));
//            	Log.i("dx : " + dx + ", width : " + childWidth);
                mCapturedView.offsetLeftAndRight(dx);//dx - childWidth);
            }
            if (dy != 0) {
    ...
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                final int pointerId = MotionEventCompat.getPointerId(ev, actionIndex);
                final float x = ev.getX();//MotionEventCompat.getX(ev, actionIndex);
                final float y = ev.getY();//MotionEventCompat.getY(ev, actionIndex);
...
//                    final int index = MotionEventCompat.findPointerIndex(ev, mActivePointerId);
                    ...
                    final float y = ev.getY();//MotionEventCompat.getY(ev, index);
                    final int idx = (int) (x - mLastX);//mLastMotionX[mActivePointerId]);
                    final int idy = (int) (y - mLastY);//mLastMotionY[mActivePointerId]);
...
//                    final int pointerCount = MotionEventCompat.getPointerCount(ev);
//                    for (int i = 0; i < pointerCount; i++) {
                        final int pointerId = 0;//MotionEventCompat.getPointerId(ev, i);
                        final float x = ev.getX();//MotionEventCompat.getX(ev, i);
                        final float y = ev.getY();//MotionEventCompat.getY(ev, i);
                        final float dx = x - mLastX;//mInitialMotionX[pointerId];
                        final float dy = y - mLastY;//mInitialMotionY[pointerId];
...
//            mCapturedView.offsetTopAndBottom(clampedY - oldTop);
        ...
        if (dx != 0 || dy != 0) {
            ...
            mCallback.onViewPositionChanged(mCapturedView, clampedX, clampedY,
                    clampedDx, clampedDy);	// logo drawable
        }
    }
